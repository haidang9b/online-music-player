<?php
	...
	if(!isset($_GET['idnghesi'])){
		die(json_encode(array('status' => false, 'message' => 'Parameters not valid')));
	}
	$idnghesi = $_GET['idnghesi'];
	
	$sql = "SELECT baihat.idbh ,`tenBH`,nghesi.tenNS,`hinhanhBH`,`linkBH` FROM `baihat` INNER JOIN nghesi on baihat.idNS = nghesi.idnghesi WHERe nghesi.idnghesi = $idnghesi";
